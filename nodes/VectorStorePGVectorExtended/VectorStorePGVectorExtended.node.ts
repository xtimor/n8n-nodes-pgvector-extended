    ...
    async execute(this: IExecuteFunctions): Promise<INodeExecutionData[][]> {
        ...
        try {
            ...
            if (mode === 'customQuery') {
                ...
                const iterationCount = embeddingInput.length;
                const baseItems = items.length ? items : embeddingInput;
                ...
                for (let itemIndex = 0; itemIndex < iterationCount; itemIndex++) {
                    const embeddingJson = embeddingInput[itemIndex].json as {
                        embedding?: number[];
                        vector?: number[];
                    };
                    ...
                    rows.forEach((row) => {
                        const baseJson = baseItems[itemIndex]?.json ?? {};
                        returnData.push({
                            json: {
                                ...baseJson,
                                id: row.id,
